...
for intf, vlan in trunk.items():
    ...
    for command in trunk_template:
        if command.endswith("allowed vlan") and ','.join(trunk[intf]).startswith('add'):
            print(f" {command} {'add '+','.join(trunk[intf][1:])}")
        elif command.endswith("allowed vlan") and ','.join(trunk[intf]).startswith('only'):
            print(f" {command} {','.join(trunk[intf][1:])}")
        elif command.endswith("allowed vlan") and ','.join(trunk[intf]).startswith('del'):
